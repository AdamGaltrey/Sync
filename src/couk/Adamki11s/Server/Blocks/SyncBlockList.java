...
public class SyncBlockList {
	...
	public List<Block> getBlockList(World w, Location corner1, Location corner2) throws SizeExceededException {
	...
	private LinkedList<Block> getBlocks(World w, int x1, int y1, int z1, int x2, int y2, int z2) throws SizeExceededException {
		AsyncSaver Saver = new AsyncSaver(w, x1, y1, z1, x2, y2, z2);
		Bukkit.getServer().getScheduler().scheduleAsyncDelayedTask(Sync.plugin, Saver, 0L);
		return Saver.getBlockList();
	}
	
	private LinkedList<Block> saveBlocks(World w, int x1, int y1, int z1, int x2, int y2, int z2) throws SizeExceededException {
		AsyncSaver Saver = new AsyncSaver(w, x1, y1, z1, x2, y2, z2);
		Bukkit.getServer().getScheduler().scheduleAsyncDelayedTask(Sync.plugin, Saver, 0L);
		return Saver.getBlockList();
	}
